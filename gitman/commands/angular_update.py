...
def angular_update(projects, ignored_dependencies, commit_message, base_dir):
    ...
    for project in project_list:
        ...
        try:
            ...
            text_result = subprocess.run(['ng', 'update'], check=True, text=True, stdout=subprocess.PIPE)
            packages = re.findall(pattern, text_result.stdout)

            if commit_message == "update: deps of project":
                package_names = [re.search(patternDep, package).group(1) for package in packages]
            ...
            subprocess.run(["ng", "update"] + packages, check=True, text=True, stdout=subprocess.PIPE)
