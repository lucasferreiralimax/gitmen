...
def ncu_update(projects, commit_message, base_dir):
    ...
    for project_dir in project_list:
        ...
        if not os.path.isdir(full_path):
            print(
                i18n.t("ncu_update.ncu_directory_not_exist").format(fullpath=full_path)
            )
            continue
        ...
        try:
            ...
            ncu_result = subprocess.run(
                ["npx", "npm-check-updates", "-u"], capture_output=True, text=True
            )
            ...
            # Exibir a saída completa para depuração
            print(
                i18n.t("ncu_update.ncu_npm_install_output").format(
                    output=install_result.stdout
                )
            )
            print(
                i18n.t("ncu_update.ncu_npm_install_errors").format(
            ...
            print(
                i18n.t("ncu_update.ncu_git_commit_message").format(
