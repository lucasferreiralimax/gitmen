...
def check_github():
    ...
    if saved_username and saved_token:
        ...
        questions = [
            inquirer.Text('username', message="Enter GitHub username", default=username),
            inquirer.Password('token', message="Enter GitHub token (leave blank for unauthenticated requests)", default=None)
        ]
...
def get_github_data(username, endpoint, token=None, page=1):
    ...
    if response.status_code == 200:
        ...
        console.print(f":x: Rate limit exceeded or authentication required for fetching {endpoint} for user {username}")
        return []
    else:
        console.print(f":x: Failed to fetch {endpoint} for user {username}. Status code: {response.status_code}")
        return []
